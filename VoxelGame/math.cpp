...
{
    float xt = static_cast<float>(x*cos(an) - z*sin(an));
    float zt = static_cast<float>(x*sin(an) + z*cos(an));
    x = xt;
    z = zt;
}
...
{
    float a = static_cast<float>(o.x);
    float b = static_cast<float>(o.y);
    float c = static_cast<float>(o.z);
    float u = static_cast<float>(eje.x);
    float v = static_cast<float>(eje.y);
    float w = static_cast<float>(eje.z);

    float u2 = u*u;
    float v2 = v*v;
    float w2 = w*w;
    float cosT = static_cast<float>(cos(theta));
    float sinT = static_cast<float>(sin(theta));
    float l2 = u2 + v2 + w2;
    float l = static_cast<float>(sqrt(l2));

    if (l2 < 0.000000001)        // el vector de rotacion es casi nulo
        return;

    float xr = static_cast<float>(a*(v2 + w2) + u*(-b*v - c*w + u*x + v*y + w*z)
        + (-a*(v2 + w2) + u*(b*v + c*w - v*y - w*z) + (v2 + w2)*x)*cosT
        + l*(-c*v + b*w - w*y + v*z)*sinT);
    xr /= l2;

    float yr = static_cast<float>(b*(u2 + w2) + v*(-a*u - c*w + u*x + v*y + w*z)
        + (-b*(u2 + w2) + v*(a*u + c*w - u*x - w*z) + (u2 + w2)*y)*cosT
        + l*(c*u - a*w + w*x - u*z)*sinT);
    yr /= l2;

    float zr = static_cast<float>(c*(u2 + v2) + w*(-a*u - b*v + u*x + v*y + w*z)
...
bool intersect(vec3 orig, vec3 dir, vec3 center, float radio)
{
    ...
    if (disc >= 0)
    {
        float t0 = (-B - static_cast<float>(sqrt(disc))) / 2;
        float t1 = (-B + static_cast<float>(sqrt(disc))) / 2;
        if (t0 > t1)
            swap(&t0, &t1);
        // t0 < t1 
